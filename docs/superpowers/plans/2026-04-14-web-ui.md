...
// NOTE: Backend is NOT in the config panel — it is embedded as a runtime
// <select> in the generated pages so users can switch backends directly.
// See inference emitters for engine-specific backend options:
// - ORT: WebNN NPU, WebNN GPU (default), WebNN CPU, WebGPU, Wasm
// - LiteRT: WebGPU (default), Wasm (WebNN options disabled/coming soon)
