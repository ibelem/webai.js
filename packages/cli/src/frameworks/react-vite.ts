...
function emitApp(config: ResolvedConfig): string {
  ...
  // Text/NLP tasks
  if (config.task === 'text-classification') return emitTextClassificationApp(config);
  if (config.task === 'zero-shot-classification') return emitZeroShotClassificationApp(config);
  if (config.task === 'text-generation') return emitTextGenerationApp(config);
