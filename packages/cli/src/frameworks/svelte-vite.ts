/**
 * Svelte + Vite framework emitter (Layer 2).
 *
 * Produces a Svelte 5 + Vite project (WITHOUT SvelteKit):
 *   package.json, vite.config.js, index.html,
 *   src/main.{ts|js}, src/App.svelte, src/App.css,
 *   src/lib/input.{ts|js} (if non-file),
 *   src/lib/preprocess.{ts|js}, src/lib/inference.{ts|js},
 *   src/lib/postprocess.{ts|js}, README.md
 *
 * Uses Svelte 5 with plain Vite. All ML inference runs client-side.
 */
...
function emitMain(config: ResolvedConfig): string {
  const bang = config.lang === 'ts' ? '!' : '';
  return `import { mount } from 'svelte';
import App from './App.svelte';
import './App.css';

mount(App, { target: document.getElementById('app')${bang} });
`;
}
...
// ---- Svelte App emitters by task/input ----

function emitFileClassificationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
...
function emitFileDetectionAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitFileSegmentationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitFileFeatureExtractionAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitFillMaskAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitSentenceSimilarityAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitDepthEstimationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- Token Classification (NER) App ----

function emitTokenClassificationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitQuestionAnsweringAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitSummarizationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitTranslationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitText2TextAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitConversationalAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitTableQAAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitImageToTextAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitVQAAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitDocQAAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- Image-Text-to-Text App ----

function emitImageTextToTextAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitAudioToAudioAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitSpeakerDiarizationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- Voice Activity Detection App ----

function emitVADAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitRealtimeAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- Audio: File + Classification App ----

function emitFileAudioClassificationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitFileSpeechToTextAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitMicSpeechToTextAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- Audio: Mic + Classification App ----

function emitMicAudioClassificationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitTextToSpeechAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitTextClassificationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- Text: Zero-Shot Classification App ----

function emitZeroShotClassificationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
function emitTextGenerationAppSvelte(config: ResolvedConfig): string {
  const le = libExt(config);
  const t = config.lang === 'ts';
  const taskLabel = getTaskLabel(config.task);
  const engineLabel = getEngineLabel(config.engine);
...
// ---- App dispatcher (switch statement) ----

function emitApp(config: ResolvedConfig): string {
  switch (config.task) {
    case 'text-to-speech': return emitTextToSpeechAppSvelte(config);
    case 'audio-classification':
      return config.input === 'mic' ? emitMicAudioClassificationAppSvelte(config) : emitFileAudioClassificationAppSvelte(config);
    case 'speech-to-text':
      return config.input === 'mic' ? emitMicSpeechToTextAppSvelte(config) : emitFileSpeechToTextAppSvelte(config);
    case 'audio-to-audio': return emitAudioToAudioAppSvelte(config);
    case 'speaker-diarization': return emitSpeakerDiarizationAppSvelte(config);
    case 'voice-activity-detection': return emitVADAppSvelte(config);
    case 'text-classification': return emitTextClassificationAppSvelte(config);
    case 'zero-shot-classification': return emitZeroShotClassificationAppSvelte(config);
    case 'text-generation': return emitTextGenerationAppSvelte(config);
    case 'fill-mask': return emitFillMaskAppSvelte(config);
    case 'sentence-similarity': return emitSentenceSimilarityAppSvelte(config);
    case 'token-classification': return emitTokenClassificationAppSvelte(config);
    case 'question-answering': return emitQuestionAnsweringAppSvelte(config);
    case 'summarization': return emitSummarizationAppSvelte(config);
    case 'translation': return emitTranslationAppSvelte(config);
    case 'text2text-generation': return emitText2TextAppSvelte(config);
    case 'conversational': return emitConversationalAppSvelte(config);
    case 'table-question-answering': return emitTableQAAppSvelte(config);
    case 'image-to-text': return emitImageToTextAppSvelte(config);
    case 'visual-question-answering': return emitVQAAppSvelte(config);
    case 'document-question-answering': return emitDocQAAppSvelte(config);
    case 'image-text-to-text': return emitImageTextToTextAppSvelte(config);
    case 'depth-estimation': return emitDepthEstimationAppSvelte(config);
    case 'object-detection':
      if (config.input === 'camera' || config.input === 'screen') return emitRealtimeAppSvelte(config);
      return emitFileDetectionAppSvelte(config);
    case 'image-segmentation':
      if (config.input === 'camera' || config.input === 'screen') return emitRealtimeAppSvelte(config);
      return emitFileSegmentationAppSvelte(config);
    case 'feature-extraction': return emitFileFeatureExtractionAppSvelte(config);
    case 'image-classification':
    default:
      if (config.input === 'camera' || config.input === 'screen') return emitRealtimeAppSvelte(config);
      return emitFileClassificationAppSvelte(config);
  }
}
...
/**
 * Emit Svelte + Vite framework files.
 */
export function emitSvelteVite(config: ResolvedConfig, blocks: CodeBlock[]): GeneratedFile[] {
  const le = libExt(config);
  ...
  const files: GeneratedFile[] = [
    ...
    { path: 'src/App.svelte', content: emitApp(config) },
